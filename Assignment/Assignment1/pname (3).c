...
pname_in(PG_FUNCTION_ARGS)
{
    ...
    //printf("result is : %s\n",result->pname);
	//memcpy(result->pname, str, strlen(str) + 1);
...
char *pname_get_part(PersonName *name, int part){
    ...
    name->pname[nameIndex] = '\0'; //Family part of pname
    if (part == 0) {
...
/**
    // Guarenteen that there are only two parts of a PersonName, family and given
    char *token[2];
    char *temp;
    int i  = 0;
    if(name->pname != NULL){
        ...
        printf("invalid pname\n");
    }

    temp=strtok(name->pname,",");
    while(temp != NULL){
        token[i] = palloc(strlen(temp)+1);
        strcpy(token[i],temp);
        i++;
        temp = strtok(NULL,",");
    }
...
pname_cmp_internal(PersonName *a, PersonName *b)
{
    ...
    //printf("family a is : %s\n",aFamily);
    //printf("family b is : %s\n",bFamily);
    if (strcmp(aFamily,bFamily) < 0) // A's family name is less than B's family name-->A's pname < B's pname
        return -1;
    if (strcmp(aFamily,bFamily) > 0) // A's family name is greater than B's family name-->A's pname > B's pname
        return 1;
    //printf("given a is : %s\n",aGiven);
    //printf("given b is : %s\n",bGiven);
...
family(PG_FUNCTION_ARGS)
{
    ...
    printf("family name is : %s\n",pname_get_part(full,0));
    PG_RETURN_TEXT_P(cstring_to_text(pname_get_part(full,0)));
    //PG_RETURN_CSTRING(pname_get_part(full,0));
}
...
given(PG_FUNCTION_ARGS)
{
    PersonName *full = (PersonName *) PG_GETARG_POINTER(0);
    printf("full name is : %s\n",full->pname);
    printf("given name is : %s\n",pname_get_part(full,1));
    PG_RETURN_TEXT_P(cstring_to_text(pname_get_part(full,1)));
    //PG_RETURN_CSTRING(pname_get_part(full,1));
}
...
{
    PersonName *pName = (PersonName *) PG_GETARG_POINTER(0);
    char *str = pName->pname;
    unsigned long length = strlen(str);

    //int hash = DatumGetUInt32(hash_any((unsigned char *) pName->pname, strlen(pName->pname)));

    PG_RETURN_INT32(DatumGetUInt32(hash_any((unsigned char *) Name->pname, length)));
}
