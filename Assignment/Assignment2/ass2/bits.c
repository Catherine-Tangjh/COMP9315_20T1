...
Bool isSubset(Bits b1, Bits b2)
{
	...
	//b1 is a subset of b2 iff the bit in b1 which is '1', is also '1' at the same position of b2
...
void setAllBits(Bits b)
{
	...
	Byte mask = ~0; // set all bits in mask to 1
...
{
	assert(b != NULL);
	Byte mask = 0; // set all bits in mask to 0
	for (int i = 0; i < b->nbytes; i++) {
		b->bitstring[i] &= mask;
	}
}
