...
int findVictim(BufPool pool)
{
    ...
    while (n < MAX_ATTEMPTS) {
        index = pool->clock;
        if (pool->bufs[index].pin == 0 && pool->bufs[index].pop == 0) {
            slot = index;
            break;
        } else if(pool->bufs[index].pop > 0) pool->bufs[index].pop--;
        if (pool->clock == pool->nbufs-1) pool->clock = 0;
        else pool->clock++;
        n++;
    }
    if (slot == NONE) return slot;
        ...
        if (pool->bufs[slot].dirty == 1) pool->nwrites++;
        return slot;
    }
}
